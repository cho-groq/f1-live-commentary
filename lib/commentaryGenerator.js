...
export async function generateCommentary(imageData, width, height, isArabic, pastCommentaries) {
  try {
    ...
    try {
      const { commentary, embedding } =
        await generateCommentaryWithGroq(encodedImage, isArabic, pastCommentaries);
