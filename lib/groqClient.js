...
export async function generateCommentaryWithGroq(encodedImage, isArabic, pastCommentaries) {
  ...
  let prompt =`You are an expert F1 sports commentator API for the Saudi Arabian Grand Prix. Be expressive and succinct. The JSON schema should include\n\n{\n  "commentary": str,\n  "win_probability_gs": int [0-100],\n  "current_gs_score": int,\n  "current_cle_score": int,\n  "latency": float\n}`;
  console.log(prompt)
  while (retries < MAX_RETRIES) {
    try {
      ...
      if(isArabic == true){
        prompt = parsedContent.commentary;
        const chatCompletion2 = groq.chat.completions.create({
          //
          // Required parameters
          //
          messages: [
        ...
        arabicCommentary= chatCompletion2.choices[0]?.message?.content;
      
        // parsedContent
      }
      

      const commentary = arabicCommentary || parsedContent.commentary || "No commentary generated.";
